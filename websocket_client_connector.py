...
# def database_manager(message: dict):
#     if message["message"] == "Save to Database":
...
#         }
#         try:
#             mongo_collection.update_one({"_id": device_name}, {"$set": data}, upsert=True)
#         except Exception as e:
...
#     print(f"Pong recieved. Message: {message}")
...
#     ws_url = "wss://15dcmwmsig.execute-api.ap-south-1.amazonaws.com/production"
...
#         on_pong=on_pong
#     )
#     ws_thread = threading.Thread(target=lambda: ws_instance.run_forever(ping_interval=540, ping_timeout=10))
